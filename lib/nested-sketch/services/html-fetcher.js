...
// Then DOMPurify it.
export async function promisePurifiedHTMLFromURL(url, htmlCache, getLocalizedText){
    // If already in cache, return that.
    if(htmlCache[url]){
        return htmlCache[url] // STOP.
    }
    ...
    let time_out_message = isWikipedia(url) ? WIKIPEDIA_TIME_OUT_MSG
                        : isYouTube(url) ? ''
                        : BASE_TIME_OUT_MSG
    ...
    try {
            ...
            fetchURL(url),
            delayTimeout(LOAD_WAIT_TIME, time_out_message)
            ]);
