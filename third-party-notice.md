## Third Party Notice

This project makes use of the following open source libraries:

### Nutshell (Modified)

* **Original Author**: Nicky Case ([ncase.me/nutshell](https://ncase.me/nutshell/))
* **Original License**: CC0 1.0 Universal (Public Domain)
* **Source**: [https://github.com/ncase/nutshell](https://github.com/ncase/nutshell)
* **Modifications**: All right reserved.
* **Note**: The code has been modified from the original version for \[Class-based triggers, Math formula rendering, Math-link, Style adjustment] and renamed nested-sketch to avoid confusion with the original version.

---

### DOMPurify

* **Copyright**: Copyright 2025 Dr.-Ing. Mario Heiderich, Cure53
* **License**: [Apache License 2.0](https://github.com/cure53/DOMPurify?tab=License-1-ov-file#readme)
* **Source**: [https://github.com/cure53/DOMPurify](https://github.com/cure53/DOMPurify)

### Marked

* **Copyright**: Copyright (c) 2018+, MarkedJS (https://github.com/markedjs/) Copyright (c) 2011-2018, Christopher Jeffrey (https://github.com/chjj/)
* **License**: [MIT License](https://github.com/markedjs/marked?tab=License-1-ov-file#readme)
* **Source**: [https://github.com/markedjs/marked](https://github.com/markedjs/marked)
* **Note**: This software includes portions of Markdown (Copyright © 2004, John Gruber), licensed under the BSD 3-Clause License.

### Katex

* **Copyright**: Copyright (c) 2013-2020 Khan Academy and other contributors
* **License**: [MIT License](https://github.com/KaTeX/KaTeX?tab=MIT-1-ov-file#readme)
* **Source**: [https://github.com/KaTeX/KaTeX](https://github.com/KaTeX/KaTeX)